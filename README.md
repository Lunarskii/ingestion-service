...
# Через python
python -m streamlin run ui/main.py
# Аналогично с запуском через poetry можете задать переменную окружения BACKEND_URL
BACKEND_URL=<backend_url> python -m streamlin run ui/main.py
```
...
# Покрытие кода
poetry run pytest --cov=api --cov=config --cov=domain --cov=exceptions --cov=services --cov=stubs --cov-report=term-missing
