    ...
    def decorator(func) -> Callable:
        ...
        def wrapper(
            ...
            **kwargs,
            ...
            if required_event_type and (event_type := event.get("type")) != required_event_type:
                raise AttributeError(f"Ожидался тип события {required_event_type}, но был получен {event_type}")
...
def on_task_started(event: dict[str, Any]) -> None:
    ...
    if task_started and task_received:
        metrics.task_prefetch_time_seconds.labels(task_name, worker_name).set(task_started - task_received)
...
def on_task_succeeded(event: dict[str, Any]) -> None:
    ...
    if task_received and task_started:
        metrics.task_runtime_seconds.labels(task_name, worker_name).observe(task_runtime)
