...
from services.celery_worker.utils import import_attribute
...
class PydanticPreserializer(Preserializer):
    ...
    def unpack(cls, data: PackedModel) -> BaseModel:
        ...
        if not issubclass(schema_type, BaseModel):
            raise TypeError(f"Нельзя распаковать {schema_type}: не Pydantic модель")
        return schema_type(**data["dump"])


def register_preserializer(
    preserializer: Preserializer, type_: type[object]
) -> type[object]:
    """
    ...
    try:
        ...
        raise TypeError(f"{type_} не совместим с {preserializer}: {e}") from e
