...
class TestFileRawStorage:
    ...
    def test_save_writes_bytes_to_correct_location(self, tmp_path, directory, path):
    ...
    def test_init_raises_for_non_directory_path(self, tmp_path, directory, path):
        directory = f"{tmp_path}/{directory}"
...
class TestJSONVectorStore:
    ...
    def test_upsert_creates_json_file_with_correct_content(
        self, tmp_path, directory, vectors
    ):
        directory = f"{tmp_path}/{directory}"
    ...
    def test_search_returns_correct_vectors(self, tmp_path):
        ...
        retrieved_vectors: list[Vector] = vector_store.search(
            question_vector, top_k, workspace_id
        )
        assert retrieved_vectors == expected_vectors

    def test_search_raises_for_empty_workspace(self, tmp_path, random_workspace_id):
        directory: str = f"{tmp_path}/{ValueGenerator.path()}"
        ...
        with pytest.raises(VectorStoreDocumentsNotFound) as exc:
            vector_store.search(vector, top_k, random_workspace_id)
...
class TestSQLiteMetadataRepository:
    ...
    def test_save_inserts_metadata_correctly(self, tmp_path, metadata_kwargs):
        ...
        cursor = db_connection.cursor()
        cursor.execute(
            f"SELECT * FROM {table_name} WHERE document_id = ?", (metadata.document_id,)
        )
        row = cursor.fetchone()
        db_connection.close()
        ...
        assert row["creation_date"] == metadata.creation_date.strftime(
            "%Y-%m-%d %H:%M:%S"
        )
